...
public class RandomBox extends Item {
	...
	public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer){
		if(DKarpov.debug) System.out.println("onItemRightClick Triggered");
		...
		if(this.costil >= 1){
			...
			if(DKarpov.debug)par3EntityPlayer.addChatMessage(new ChatComponentText("§r§1[§6" + par3EntityPlayer.getGameProfile().getName() +
																					"§1] §r§2Получил " + obj.getDisplayName() + 
																					" в количестве " + count));
			this.costil = 0;	
		}else{
			this.costil++;
